...
export default async function(
    ...
    // Promises generator
    let axiosWithConfig = null;
    
    try {

        axiosWithConfig = create( options.axios );
        ...
        ! options.hideErrorsInConsole && logger.error(
            '\x1B[31m%s\x1B[0m',
            moduleName,
            'creation of axios failed, this could be a network problem',
            e,
        );
    ...
    if( ! axiosWithConfig ) {
         ...
         ! options.hideErrorsInConsole && logger.info(
            '\x1B[32m%s\x1B[0m',
            moduleName,
            'no Axios config found, the last generated `configuration-file.json` will be used',
            'Exit.',
        );
    ...
    // Requests filtering
    const validRequests = options.requests.filter(
            obj => ! obj.skip,
        )
        , {
            requestsInParallel,
